...
if ( ! function_exists( 'smarter_foundation_post_nav' ) ) :
/**
 * Display navigation to next/previous post when applicable.
 */
function smarter_foundation_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'smarter_foundation' ); ?></h1>
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', _x( '<span class="meta-nav">&larr;</span> %title', 'Previous post link', 'smarter_foundation' ) );
				next_post_link(     '<div class="nav-next">%link</div>',     _x( '%title <span class="meta-nav">&rarr;</span>', 'Next post link',     'smarter_foundation' ) );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
if ( ! function_exists( 'smarter_foundation_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function smarter_foundation_posted_on() {
	...
	printf( __( '<span class="posted-on">Posted on %1$s</span><span class="byline"> by %2$s</span>', 'smarter_foundation' ),
 ...
 * @return bool
 */
function smarter_foundation_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'smarter_foundation_categories' ) ) ) {
		...
		set_transient( 'smarter_foundation_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so smarter_foundation_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so smarter_foundation_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in smarter_foundation_categorized_blog.
 */
function smarter_foundation_category_transient_flusher() {
	// Like, beat it. Dig?
	delete_transient( 'smarter_foundation_categories' );
}
add_action( 'edit_category', 'smarter_foundation_category_transient_flusher' );
add_action( 'save_post',     'smarter_foundation_category_transient_flusher' );
